...
const southSoutheastOrigins = new Set(['MG', 'PR', 'RJ', 'RS', 'SC', 'SP']);
const sevenPercentDestinations = new Set(['AC', 'AL', 'AP', 'AM', 'BA', 'CE', 'DF', 'ES', 'GO', 'MA', 'MT', 'MS', 'PA', 'PB', 'PE', 'PI', 'RN', 'RO', 'RR', 'SE', 'TO']);
const interstateRateFor = (origin: string, destination: string) => southSoutheastOrigins.has(origin) && sevenPercentDestinations.has(destination) ? 7 : 12;
...
export default function EntradaDifal() {
  ...
  const totals = useMemo(() => items.filter((item) => item.selected).reduce((total, item) => {
    const source = item.value * interstateRateFor(item.issuerUf, item.destinationUf) / 100; const destination = item.value * internalRate / 100;
  ...
  }, { items: 0, base: 0, source: 0, destination: 0, difal: 0 }), [items, internalRate]);
  ...
  return <div className="mx-auto max-w-[1500px] space-y-6 px-6 py-8">
    ...
    <section className="rounded-xl border border-sky-500/30 bg-slate-900/70 p-5"><div className="grid gap-4 lg:grid-cols-[170px_220px_1fr]"><label className="bk-label">Estado de destino<select value={destinationUf} onChange={(event) => setDestinationUf(event.target.value)} className="bk-input">{states.map((state) => <option key={state}>{state}</option>)}</select></label><label className="bk-label">Alíquota interna destino (%)<input value={internalText} inputMode="decimal" onChange={(event) => setInternalText(event.target.value)} className="bk-input" /></label><label className="flex cursor-pointer items-center justify-center gap-2 rounded-lg border border-dashed border-slate-600 bg-slate-950/40 px-4 py-3 text-sm text-sky-300 hover:border-sky-400"><FileUp className="h-4 w-4" /><span>{ncmFileName || 'Carregar tabela NCM (.json)'}</span><input type="file" accept="application/json,.json" onChange={loadNcmTable} className="hidden" /></label></div><p className="mt-3 text-xs text-slate-400">A alíquota interestadual é definida automaticamente pela UF de origem e destino de cada XML: 7% nas rotas aplicáveis de Sul/Sudeste (exceto ES) para Norte, Nordeste, Centro-Oeste e ES; 12% nas demais rotas interestaduais.</p></section>
    ...
    <section className="overflow-hidden rounded-xl border border-slate-700 bg-slate-900/70"><div className="flex flex-wrap items-center justify-between gap-3 border-b border-slate-700 p-4"><div><h2 className="font-semibold text-white">Itens importados para conferência</h2><p className="text-xs text-slate-400">Clique na seleção para definir quais NCMs gerarão o imposto.</p></div><div className="flex gap-2"><button onClick={() => selectByRule(false)} className="bk-button-secondary text-xs"><CheckSquare className="h-4 w-4" />Selecionar sem alerta</button><button onClick={() => selectByRule(true)} className="bk-button-secondary text-xs"><CheckSquare className="h-4 w-4" />Selecionar alertas</button></div></div><div className="overflow-x-auto"><table className="bk-table min-w-[1430px]"><thead><tr><th>Gerar</th><th>Nota</th><th>Origem / destino</th><th>Alíquota interestadual</th><th>CFOP</th><th>NCM</th><th>Descrição NCM</th><th>Produto XML</th><th>Valor</th><th>Anexo XXI</th><th>DIFAL</th></tr></thead><tbody>{items.length ? items.map((item) => { const interstateRate = interstateRateFor(item.issuerUf, item.destinationUf); const source = item.value * interstateRate / 100; const destination = item.value * internalRate / 100; const difal = Math.max(0, destination - source); return <tr key={item.id} className={item.selected ? '' : 'opacity-60'}><td><button title={item.selected ? 'Não gerar DIFAL' : 'Gerar DIFAL'} onClick={() => toggle(item.id)} className="text-sky-300">{item.selected ? <CheckSquare className="h-5 w-5" /> : <Square className="h-5 w-5" />}</button></td><td title={item.fileName}>{item.noteNumber}</td><td>{item.issuerUf} → {item.destinationUf}</td><td className="font-semibold text-sky-300">{interstateRate}%</td><td>{item.cfop}</td><td className="font-mono">{item.ncm || '—'}</td><td className="max-w-80 whitespace-normal">{item.ncmDescription}</td><td className="max-w-64 whitespace-normal">{item.description}</td><td>{money(item.value)}</td><td>{item.annexAlert ? <span className="flex items-center gap-1 text-amber-300"><AlertTriangle className="h-4 w-4" />Conferir retenção</span> : <span className="text-emerald-300">Sem alerta</span>}</td><td className="font-semibold text-sky-300">{money(difal)}</td></tr>; }) : <tr><td colSpan={11} className="py-12 text-center text-slate-500">Carregue a tabela NCM e selecione os XMLs para iniciar a conferência.</td></tr>}</tbody></table></div></section>
